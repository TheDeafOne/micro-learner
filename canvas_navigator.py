...
def find_ai_course():
    courses = get_all(urljoin(BASE, "/api/v1/courses"),
                      params={"per_page": 100})
    for c in courses:
...
def find_module(course_id, n):
    mods = get_all(urljoin(BASE, f"/api/v1/courses/{course_id}/modules"),
                   params={"per_page": 100})
    ...
    for m in mods:
        if (m.get("name", "").lower().startswith(want_prefix)):
            return m["id"], m["name"]
    sys.exit(f"Module {n} not found. Available: " +
             ", ".join(m.get("name", "") for m in mods))


def find_lectures_page(course_id, module_id, n):
    items = get_all(urljoin(BASE, f"/api/v1/courses/{course_id}/modules/{module_id}/items"),
                    params={"per_page": 100})
    # First try exact "Module N - Lectures", else any Page with "Lectures"
    exact = f"module {n} - lectures"
    page = next((i for i in items
                 if i.get("type") == "Page" and i.get("title", "").lower() == exact), None)
    if not page:
        page = next((i for i in items
                     if i.get("type") == "Page" and "lectures" in i.get("title", "").lower()), None)
    if not page:
...
def get_page_body(course_id, page_url):
    r = requests.get(urljoin(BASE, f"/api/v1/courses/{course_id}/pages/{page_url}"),
                     headers=HEADERS)
    r.raise_for_status()
...
async def main():
    ...
    summarize_prompt = '''
        ...
        Just provide the summarized information in a concise format. Here is the transcript:
        {transcript_text}
    '''
    summary_prompts = [summarize_prompt.format(
        transcript_text=t) for t in transcripts]
    responses = await multi_query(summary_prompts)

    out_path = f'{module_id}_lecture_notes.txt'.replace(' ', '_')
